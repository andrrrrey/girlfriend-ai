...
import { admin, type PoseCategory, type PoseOption } from "../../../lib/api";
...
export default function AdminPoseOptionsPage() {
  ...
  const [activeTab, setActiveTab] = useState("FACIAL_EXPRESSION");
  const [categories, setCategories] = useState<PoseCategory[]>([]);
  const [expandedCatId, setExpandedCatId] = useState<string | null>(null);
  const [optionsByCat, setOptionsByCat] = useState<Record<string, PoseOption[]>>({});

  const [showCatForm, setShowCatForm] = useState(false);
  const [editingCat, setEditingCat] = useState<PoseCategory | null>(null);
  ...
  const [editingOpt, setEditingOpt] = useState<PoseOption | null>(null);
  const [optForm, setOptForm] = useState<OptForm>(emptyOptForm);
  const [optError, setOptError] = useState("");
  const [savingOpt, setSavingOpt] = useState(false);

  const loadCategories = () => {
    admin.getPoseCategories(activeTab).then((cats) => {
      setCategories(cats);
  ...
  const loadOptions = (categoryId: string) => {
    admin.getPoseOptions(categoryId).then((opts) => {
  ...
  const openEditCat = (cat: PoseCategory, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingCat(cat);
  ...
  const handleSaveCat = async () => {
    ...
    try {
      ...
      if (editingCat) {
        await admin.updatePoseCategory(editingCat.id, data);
      } else {
        await admin.createPoseCategory(data);
      }
  ...
  const handleDeleteCat = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      await admin.deletePoseCategory(id);
  ...
  const handleSaveOpt = async () => {
    ...
    try {
      ...
      if (editingOpt) {
        ...
        await admin.createPoseOption(data);
      }
      setShowOptForm(null);
      setEditingOpt(null);
  ...
  const handleDeleteOpt = async (opt: PoseOption, e: React.MouseEvent) => {
    e.stopPropagation();
        ...
        {showCatForm && (
          <div style={s.formBox}>
            ...
            <div style={s.formRow}>
              <input
                style={s.input}
                placeholder="Category name (e.g. Neutral)"
                value={catForm.name}
                    ...
                    {showOptForm === cat.id && (
                      <div style={{ ...s.formBox, background: "#111", marginTop: 0, marginBottom: 12 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          ...
                          />
                          <label htmlFor="pose-opt-img" style={{ ...s.cancelBtn, cursor: "pointer" }}>
